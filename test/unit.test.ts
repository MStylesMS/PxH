...
describe('processMatch', () => {
  it('matches known unit cmdlines', () => {
    ...
    assert.equal(
      cmdlineMatchesUnit('pxm', 'node /opt/paradox/apps/PxM/src/pxm.js --config /opt/paradox/config/pxm.ini'),
      true,
    );
    assert.equal(
      cmdlineMatchesUnit('tfd-pxm', '/usr/local/bin/node /opt/paradox/apps/PxM/src/pxm.js --config /x'),
      true,
    );
